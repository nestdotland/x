...
function split(encrypted: string) {
  ...
  if (!constants.ENCRYPTION_ALGOS[algo])
    throw new Error("Invalid algorithm identifier");
  let [iv, tag, encryptedToken, hashedToken] = splitEnc[1]
    .split(":")
...
function join({
  ...
  let data = `${iv.toString("base64")}:${tag.toString(
    "base64",
  )}:${encryptedToken.toString("base64")}:${hashedToken.toString(
    "base64",
  )}`.replace(/=+/g, "");
  return `$${algo}$${data}$`;
}
...
async function _encrypt(token: string, key: string) {
  ...
  if (crypto.getCiphers().indexOf(algoSpecs.algoName) === -1)
    throw new Error("Invalid encryption algorithm");
  if (crypto.getHashes().indexOf(algoSpecs.hashName) === -1)
    throw new Error("Invalid hashing algorithm");
  ...
  let hashedToken = crypto
    .createHash(algoSpecs.hashName)
    .update(iv)
    .update(tokenBuffer)
    .digest()
    .slice(0, algoSpecs.hashLength);
  if (derivedKey.length !== algoSpecs.keyLength)
    throw new Error("Hash not long enough");
  ...
  let tag = crypto
    .createHash(algoSpecs.hashName)
    .update(constants.DEFAULT_ENCRYPTION_ALGO)
    .update(derivedKey)
    .update(iv)
    .update(encryptedToken)
    .digest()
    .slice(0, algoSpecs.tagLength);
  let outString = join({
    algo: constants.DEFAULT_ENCRYPTION_ALGO,
    iv: iv,
    tag: tag,
...
async function _decrypt(
  ...
  if (crypto.getHashes().indexOf(algoSpecs.hashName) === -1)
    throw new Error("Invalid hashing algorithm");
  let derivedKey = (await _pbkdf2Key(key, parsedToken.iv)).slice(
    0,
    algoSpecs.keyLength,
  );
  if (derivedKey.length !== algoSpecs.keyLength)
  ...
  let tag = crypto
    .createHash(algoSpecs.hashName)
    .update(parsedToken.algo)
    .update(derivedKey)
    .update(parsedToken.iv)
    .update(parsedToken.encryptedToken)
    .digest()
    .slice(0, algoSpecs.tagLength);
...
function _verify(encryptedToken: string, toCompare: string): boolean {
  ...
  let hashedToken = crypto
    .createHash(algoSpecs.hashName)
    .update(parsedToken.iv)
    .update(Buffer.from(toCompare, "utf8"))
    .digest()
    .slice(0, algoSpecs.hashLength);
